...
# 1. Initialize the camera (Fixes NameError)
video_capture = cv2.VideoCapture(0)

# 2. Setup the save path automatically
# This points to: livenessdetect/dataset/real/
save_path = os.path.join(os.path.dirname(__file__), "..", "dataset", "real")
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if(x<800 and x>400 and y<300 and y>100 and (x+w)<900 and (x+w)>400 and (y+h)<560 and len(faces)==1):
            ...
            # Save the image
            filename = "real%d.jpg" % random.randint(40000, 90000)
